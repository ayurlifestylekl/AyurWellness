...
import ApproveRejectBar from '../../marketplace-orders/[id]/ApproveRejectBar'

export const metadata = { title: 'Agent Submission · Admin' }
...
export default async function AgentSubmissionDetailPage({
  ...
  if (m.entered_by_admin_id) {
    redirect(`/admin/marketplace-orders/${m.id}`)
  }
        ...
        <section className="rounded-2xl border border-[#B58A3B]/30 bg-[#EDF4E7]/50 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 font-heading text-[16px] font-bold text-[#006B3C]">
                {agentUser?.full_name ?? '—'}
              </p>
              <p className="mt-0.5 text-[12px] text-[#12372D]/65">
                <code className="font-mono font-semibold text-[#B58A3B]">{agent.referral_code}</code>
                {agentUser?.email ? <> · {agentUser.email}</> : null}
              </p>
            </div>
            <Link
              href={`/admin/partners/${agent.id}`}
              className="rounded-lg border border-[#006B3C]/15 bg-white px-3 py-1.5 text-[11.5px] font-semibold text-[#006B3C] hover:bg-[#EDF4E7]"
            >
              View affiliate →
            </Link>
          </div>
          <p className="mt-3 text-[11.5px] italic text-[#12372D]/65">
            Approving creates a real order in /admin/orders, deducts stock, and credits this
            affiliate&apos;s commission automatically.
          </p>
        </section>
      ) : null}
        ...
        <article className="rounded-2xl border border-[#006B3C]/8 bg-white p-4 lg:col-span-2">
          ...
          {m.notes ? (
            <div className="mt-3 rounded-lg border border-[#006B3C]/8 bg-[#EDF4E7]/30 p-3">
              ...
              <p className="mt-1 text-[12px] italic text-[#12372D]/70">{m.notes}</p>
            </div>
          ) : null}
        </article>
